...
import { CustomError, ErrorType, handleErrorResponse } from '../db/errorHelper';
...
export class UserController extends ControllerBase {
  constructor() {
    ...
    public async forgotPassword(req: NextApiRequest, res: NextApiResponse) {
      try {
        const { email } = req.body as { email: string };
  
        // Generate reset token and send it to the user's email
        const resetToken = await resetPasswordService.generateResetToken(email);
  
        // Respond with success message
        res.status(200).json({ message: 'Reset token sent successfully', resetToken });
      } catch (error) {
        handleErrorResponse(res, new CustomError(ErrorType.InternalServerError, 'Failed to send reset token'));
      }
    }

    public async resetPassword(req: NextApiRequest, res: NextApiResponse) {
      try {
        const { token, newPassword } = req.body as { token: string; newPassword: string };
  
        // Verify reset token and get user information
        const { userId, email } = await resetPasswordService.verifyResetToken(token, req, res);
  
        // Reset the user's password
        await resetPasswordService.resetPassword(userId, newPassword);
  
        // Respond with success message
        res.status(200).json({ message: 'Password reset successfully' });
      } catch (error) {
        handleErrorResponse(res, new CustomError(ErrorType.InternalServerError, 'Failed to reset password'));
      }
    }
